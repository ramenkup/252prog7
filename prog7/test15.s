# Changes the $s registers used by main; parens should not be relying on the $s registers
#    used by main.
...
main:
         ...
         lw    $s4, 0($t0)        # $s4 = number of strings
         la    $s5, mainFormulas  # $s5 = addr mainFormulas[0]
mainLoopBegin:
         slt   $t0, $s3, $s4      # $t0 = i < mainNumFormulas
         ...
         # print the string
         lw    $a0, 0($s5)        # $s4 = addr of start of current string
         addi  $v0, $zero, 4
         syscall
         ...
         # Put something in $s0, $s1, $s2, $s6, and $s7 to test correctness of procedure
         # call conventions.
         ...
         lw    $a0, 0($s5)        # $a0 = addr of string start
...
mainSDone:
         ...
         addi  $v0, $zero, 4
         syscall
         lw    $a0, 0($s5)        # $a0 = addr of formula start
         addi  $v0, $zero, 4
         syscall
         la    $a0, mainNewline
         addi  $v0, $zero, 4
...
mainAfterBalance:
         ...
         addi  $s3, $s3, 1        # i++
         addi  $s5, $s5, 4        # $s5 = addr of next string
         j     mainLoopBegin
